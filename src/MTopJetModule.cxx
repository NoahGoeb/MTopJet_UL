 ...
 */
class MTopJetModule: public AnalysisModule {
public:
    
    explicit MTopJetModule(Context & ctx);
...
MTopJetModule::MTopJetModule(Context & ctx){
    // In the constructor, the typical tasks are to initialize the
    // member variables, in particular the AnalysisModules such as
    // CommonModules or some cleaner module, Selections and Hists.
    // But you can do more and e.g. access the configuration, as shown below.
    
    cout << "Hello World from MTopJetModule!" << endl;
    ...
    dijet_sel.reset(new DijetSelection()); // see MTopJetSelections

    // 3. Set up Hists classes:
    h_nocuts.reset(new MTopJetHists(ctx, "NoCuts"));
    h_njet.reset(new MTopJetHists(ctx, "Njet"));
    h_dijet.reset(new MTopJetHists(ctx, "Dijet"));
    h_ele.reset(new ElectronHists(ctx, "ele_nocuts"));
}


bool MTopJetModule::process(Event & event) {
    ...
    cout << "MTopJetModule: Starting to process event (runid, eventid) = (" << event.run << ", " << event.event << "); weight = " << event.weight << endl;
...
// make sure the MTopJetModule is found by class name. This is ensured by this macro:
UHH2_REGISTER_ANALYSIS_MODULE(MTopJetModule)
